...
merge multiple `.mbtiles` files into a single file. It takes 80% less time than
...
to its credit, lets you use up to 20 multiple runners simultaneously. So even though
...
With the approach above, my Nova Scotia test case completes in ~7 seconds -- a nice speed up!

Even better, my production use case of North America completes in 25 minutes (down from 130 minutes).
