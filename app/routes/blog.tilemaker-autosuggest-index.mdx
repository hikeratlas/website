...
async function main() {
  ...
  const stmt = db.query(`INSERT INTO items (qrank, name, lon, lat) VALUES ($qrank, $name, $lon, $lat)`);
...
def search(q):
    ...
    cur = con.cursor()
    cur.execute("SELECT * FROM items WHERE rowid IN (SELECT rowid FROM fts WHERE name MATCH ?) ORDER BY qrank DESC LIMIT 10", (q,))
